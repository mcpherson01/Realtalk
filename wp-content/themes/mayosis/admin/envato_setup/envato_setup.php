...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function init_globals() {
            ...
            $this->envato_username = apply_filters( $this->theme_name . '_theme_setup_wizard_username', 'teconcetheme' );
            $this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', 'http://www.teconce.download/envato/api/server-script.php' );
        ...
        public function admin_menus() {
			add_submenu_page('mayosis-admin-menu', __( 'Setup Wizard','mayosis' ), __( 'Setup Wizard','mayosis' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
		}
        ...
        public function init_wizard_steps() {
            ...
            $this->steps = array(
                'introduction' => array(
                    'name'    => esc_html__( 'Introduction','mayosis' ),
        ...
        public function setup_wizard() {
            ...
            wp_localize_script( 'envato-setup', 'envato_setup_params', array(
                ...
                'verify_text'      => esc_html__( '...verifying','mayosis' ),
            ) );
    ...
    public function setup_wizard_header() {
        ...
        <head>
            ...
            echo '<t';
            echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard','mayosis' ) . '</ti' . 'tle>'; ?>
        ...
        <?php if ( 'next_steps' === $this->step ) : ?>
            <a class="wc-return-to-dashboard"
               href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard','mayosis' ); ?></a>
        <?php endif; ?>
        </body>
        <?php
        ...
        public function envato_setup_introduction() {
            ...
            if ( false && isset( $_REQUEST['debug'] ) ) {
                ...
                ?>
                <h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','mayosis' ), wp_get_theme() ); ?></h1>
                <p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','mayosis' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!','mayosis' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button button-large"><?php esc_html_e( 'Not right now','mayosis' ); ?></a>
                </p>
                <?php
            } else if ( get_option( 'envato_setup_complete', false ) ) {
                ?>
                <h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','mayosis' ), wp_get_theme() ); ?></h1>
                <p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ','mayosis' ); ?></p>
                <ul>
                    <li>
                        <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                           class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','mayosis' ); ?></a>
                    </li>
                </ul>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button button-large"><?php esc_html_e( 'Cancel','mayosis' ); ?></a>
                </p>
                <?php
            } else {
                ?>
                <h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.','mayosis' ), wp_get_theme() ); ?></h1>
                <p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.','mayosis' ), wp_get_theme() ); ?></p>
                <p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!','mayosis' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!','mayosis' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
        ...
        public function envato_setup_introduction_save() {
            ...
            if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.','mayosis' ); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        public function envato_setup_default_plugins() {
            ...
            ?>
            <h1><?php esc_html_e( 'Default Plugins','mayosis' ); ?></h1>
            <form method="post">
                ...
                if ( count( $plugins['all'] ) ) {
                    ?>
                    <p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:','mayosis' ); ?></p>
                    ...
                    echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.','mayosis' ) . '</strong></p>';
                } ?>

                <p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.','mayosis' ); ?></p>

                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"
                       data-callback="install_plugins"><?php esc_html_e( 'Continue','mayosis' ); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step','mayosis' ); ?></a>
        ...
        public function ajax_plugins() {
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found','mayosis' ) ) );
            }
            $json = array();
            ...
            foreach ( $plugins['activate'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Activating Plugin','mayosis' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['update'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Updating Plugin','mayosis' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['install'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Installing Plugin','mayosis' ),
                    );
                    break;
                }
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','mayosis' ) ) );
            }
            exit;
        ...
        private function _content_default_get() {
            ...
            foreach ( $available_content as $post_type => $post_data ) {
                if ( count( $post_data ) ) {
                    ...
                    $content[ $post_type ] = array(
                        ...
                        'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.','mayosis' ), $post_type_title ),
                        'pending'          => esc_html__( 'Pending.','mayosis' ),
                        'installing'       => esc_html__( 'Installing.','mayosis' ),
                        'success'          => esc_html__( 'Success.' ,'mayosis'),
            ...
            $content['widgets'] = array(
                'title'            => esc_html__( 'Widgets','mayosis' ),
                'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.' ,'mayosis'),
                'pending'          => esc_html__( 'Pending.' ,'mayosis'),
                'installing'       => esc_html__( 'Installing Default Widgets.','mayosis' ),
                'success'          => esc_html__( 'Success.','mayosis' ),
            ...
            $content['settings'] = array(
                'title'            => esc_html__( 'Settings' ,'mayosis'),
                'description'      => esc_html__( 'Configure default settings.' ,'mayosis'),
                'pending'          => esc_html__( 'Pending.','mayosis' ),
                'installing'       => esc_html__( 'Installing Default Settings.' ,'mayosis'),
                'success'          => esc_html__( 'Success.','mayosis' ),
        ...
        public function envato_setup_default_content() {
            ?>
            <h1><?php esc_html_e( 'Default Content','mayosis' ); ?></h1>
            <form method="post">
                <?php if ( $this->is_possible_upgrade() ) { ?>
                    <p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','mayosis' ); ?></p>
                <?php } else { ?>
                    <p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ','mayosis' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
                <?php } ?>
                <table class="envato-setup-pages" cellspacing="0">
                    <thead>
                    <tr>
                        <td class="check"></td>
                        <th class="item"><?php esc_html_e( 'Item','mayosis' ); ?></th>
                        <th class="description"><?php esc_html_e( 'Description','mayosis' ); ?></th>
                        <th class="status"><?php esc_html_e( 'Status','mayosis' ); ?></th>
                    </tr>
                    </thead>
                    <tbody>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"
                       data-callback="install_content"><?php esc_html_e( 'Continue','mayosis' ); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step','mayosis' ); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
        ...
        public function ajax_content() {
            ...
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found' ,'mayosis') ) );
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array(
                    'error'   => 1,
                    'message' => esc_html__( 'Error','mayosis' ),
                    'logs'    => $this->logs,
                    'errors'  => $this->errors,
                ) );
            }
        ...
        private function _fetch_remote_file( $url, $post ) {
            ...
            if ( ! $upload || $upload['error'] ) {
                ...
                if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond','mayosis' ) );
                }
                ...
                if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size' ,'mayosis') );
                }

                if ( 0 == $filesize ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded','mayosis' ) );
                }

                if ( ! empty( $max_size ) && $filesize > $max_size ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s','mayosis' ), size_format( $max_size ) ) );
                }
            }
        ...
        public function envato_license() {
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','mayosis' ); ?></a>
            </p>
        ...
        public function envato_setup_customize() {
            ...
            <ul>
                <li>Styles: Whole Site global colors & styles.
                </li>
                <li>Header: You can manage all of the header options from here!</li>
                <li>Product Options: Manage Easy Digital Download Product Options</li>
                <li>Footer: Here you will get all footer options!</li>
                <li>Typography: Control the site Typography!</li>
                <li>Template: Change Site Products & Blog single post Design</li>
                <li>White label: Customize the Wordpress Login</li>
            </ul>
            <p>To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars. More details in
                documentation.</p>
            <p>
                <em>Advanced Users: If you are going to make changes to the theme source code please use a <a
                    ...
                    overwriting your source code changes. <br/> See <code>mayosis-child.zip</code> in the main theme folder.</em>
            </p>
        ...
        public function envato_setup_help_support() {
            ...
            <p class="greendata">Item Support can be accessed from <a href="https://teconce.com/support/" target="_blank">Teconce help</a>
                and includes:</p>
            <ul>
            ...
            <p class="reddata">Item Support <strong>DOES NOT</strong> Include:</p>
            <ul>
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue','mayosis' ); ?></a>
                <?php wp_nonce_field( 'envato-setup' ); ?>
            </p>
            <?php
        }
        ...
        public function envato_setup_ready() {
            ...
            <a href="https://twitter.com/share" class="twitter-share-button"
               data-url="https://themeforest.net/user/teconcetheme/portfolio"
            ...
            <h1><?php esc_html_e( 'Your Website is Ready!','mayosis' ); ?></h1>

            <p class="greendata">Congratulations! The theme has been activated and your website is ready. Login to your WordPress
            ...
            <p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
                if you are happy with this theme. <br/>Follow <a href="https://twitter.com/teconce" target="_blank">@teconce</a>
                on Twitter to see updates. Thanks! </p>

            <div class="envato-setup-next-steps">
                <div class="envato-setup-next-steps-first">
                    <h2><?php esc_html_e( 'Next Steps','mayosis' ); ?></h2>
                    <ul>
                        <li class="setup-product"><a class="button button-primary button-large"
                                                     href="https://facebook.com/teconce"
                                                     target="_blank"><?php esc_html_e( 'Follow Teconce on Facebook','mayosis' ); ?></a>
                        </li>
                        <li class="setup-product"><a class="button button-next button-large"
                                                     href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!','mayosis' ); ?></a>
                        </li>
                    </ul>
                </div>
                <div class="envato-setup-next-steps-last">
                    <h2><?php esc_html_e( 'More Resources','mayosis' ); ?></h2>
                    <ul>
                        <li class="documentation"><a href="https://teconce.com/support/docs/mayosis/"
                                                     target="_blank"><?php esc_html_e( 'Read the Theme Documentation','mayosis'); ?></a>
                        </li>
                        <li class="howto"><a href="https://wordpress.org/support/"
                                             target="_blank"><?php esc_html_e( 'Learn how to use WordPress','mayosis' ); ?></a>
                        </li>
                        <li class="rating"><a href="http://themeforest.net/downloads"
                                              target="_blank"><?php esc_html_e( 'Leave an Item Rating','mayosis' ); ?></a></li>
                        <li class="support"><a href="https://teconce.com/support/"
                                               target="_blank"><?php esc_html_e( 'Get Help and Support','mayosis' ); ?></a></li>
                    </ul>
                </div>
            </div>
            <?php
        ...
        public function envato_market_admin_init() {
            ...
            if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
                ...
                if ( is_wp_error( $response ) ) {
                    ...
                    $class         = 'error';
                    echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s','mayosis' ), $error_message ) . '</p></div>';
                } else {
            ...
            add_settings_section(
                ...
                sprintf( esc_html__( 'Login for %s updates','mayosis' ), $this->envato_username ),
            ...
            add_settings_field(
                ...
                esc_html__( 'oAuth Login','mayosis' ),
                ...
                if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
                    ...
                    if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            <p><?php
                                _e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.','mayosis' );
                                ?></p>
                            <p>
                                <?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>','mayosis' ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
                            </p>
                        </div>
