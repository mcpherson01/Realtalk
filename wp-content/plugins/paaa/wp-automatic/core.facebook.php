		...
		if(  (trim($cg_fb_page_id) != '' )   ){
			...
			if (  (stristr($exec, 'logout.php')  && stristr($exec, $cg_fb_page_id)) || ( stristr($exec, 'logout.php') && $cg_fb_from == 'events' )  ){ // Checks that the object is created correctly
				...
				foreach ( $items as $item ){
					...
					if( $cg_fb_from == 'events' ){
						preg_match_all('!>Details</div></div></div><div class=".*?">(.*?)</div>!s', $item,$contMatches);
...
<div class="fb-post" data-href="https://www.facebook.com/'.$cg_fb_page_id.'/posts/'. $single_id .'"></div>';
				...
				if(in_array('OPT_FB_CACHE', $camp_opt)){
					...
					$nextPageUrl = '';
					if(stristr($exec , '<div class="i">') || stristr($exec, '"/groups/'. $cg_fb_page_id .'?bacr=') || stristr($exec, '/profile/timeline/stream/')  || stristr($exec,'See More Events')  ){
						
						if( stristr($exec, 'See More Events')  ){
							...
							preg_match('{<a href="([^"]*?)"><span>See More Event}s', $exec,$next_page_matches);
