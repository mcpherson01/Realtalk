...
class CBC_Shortcodes{
	...
	private function _shortcodes( $shortcode = '' ){
		$shortcodes = array( 
				...
				'cbc_video' => array( 
						...
						'atts' => array( 
								'id' => array( 
										'description' => __( 'WP video post ID (required)', 'cbc_video' ), 
										'value' => false 
								), 
								'controls' => array( 
										'description' => __( 'Display controls on video player ( values: 0 or 1 )', 'cbc_video' ), 
										'value' => 1 
								), 
								'autoplay' => array( 
										'description' => __( 'Autoplay video ( value: 0 or 1 )', 'cbc_video', 'cbc_video' ), 
										'value' => 0 
								), 
								'volume' => array( 
										'description' => __( 'Playback volums ( value between 0 and 100 )', 'cbc_video' ), 
										'value' => 30 
								), 
								'width' => array( 
										'description' => __( 'Player width ( value in pixels )', 'cbc_video' ), 
										'value' => 600 
								), 
								'aspect_ratio' => array( 
										'description' => __( 'Video player aspect ratio ( value: 16x9 or 4x3 )', 'cbc_video' ), 
										'value' => '16x9' 
								) 
						) 
				), 
				...
				'cbc_playlist' => array( 
						...
						'atts' => array( 
								'videos' => array( 
										'description' => __( 'Comma separated list of WP video post IDs ( ie: 12,15,29,44 ) ', 'cbc_video' ), 
										'value' => '' 
								) 
						) 
				) 
	...
	private function _shortcode_defaults( $shortcode ){
		...
		if( ! $data ){
			trigger_error( sprintf( __( 'Unknown shortcode %s.', 'cbc_video' ), $shortcode ), E_USER_NOTICE );
			return;
		}
	...
	public function shortcode_video( $atts, $content = '' ){
		...
		$video_opt = ccb_get_video_settings( $atts[ 'id' ] );
		// get video data
		$video = $this->obj->get_post_video_data( $atts['id'] );
		
		if( ! $video ){
			if( current_user_can( 'edit_posts' ) ){
				return '<span style="color:red;">' . sprintf( __( 'Shortcode error: Video post ID %d not found.', 'cbc_video' ), $atts[ 'id' ] ) . '</span>';
			}
