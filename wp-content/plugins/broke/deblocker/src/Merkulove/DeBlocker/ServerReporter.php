...
final class ServerReporter {
	...
	public function get_fields() {
		return [
			'os'                    => esc_html__( 'Operating System', 'deblocker' ),
			'software'              => esc_html__( 'Software','deblocker' ),
			'mysql_version'         => esc_html__( 'MySQL version','deblocker' ),
			'php_version'           => esc_html__( 'PHP Version','deblocker' ),
			'write_permissions'     => esc_html__( 'Write Permissions','deblocker' ),
			'zip_installed'         => esc_html__( 'ZIP Installed','deblocker' ),
			'curl_installed'        => esc_html__( 'cURL Installed','deblocker' ),
			'dom_installed'         => esc_html__( 'DOM Installed','deblocker' ),
			'xml_installed'         => esc_html__( 'XML Installed','deblocker' ),
			/** 'bcmath_installed'      => esc_html__( 'BCMath Installed','deblocker' ), */
			'allow_url_fopen'       => esc_html__( 'allow_url_fopen','deblocker' ),
		];
	}
	...
	public function get_php_version() {
		...
		if ( version_compare( $result['value'], '5.6', '<' ) ) {
			$result['recommendation'] = esc_html__( 'We recommend to use php 5.6 or higher', 'deblocker' );
	...
	public function get_zip_installed() {
		...
		return [
			'value' => $zip_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
	...
	public function get_curl_installed() {
		...
		return [
			'value' => $curl_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
			'warning' => ! $curl_installed,
			'recommendation' => esc_html__('You must enable CURL (Client URL Library) in PHP. Contact the support service of your hosting provider. They know what to do.', 'deblocker' )
		];
	}
	...
	public function get_dom_installed() {
		...
		return [
			'value' => $dom_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
			'warning' => ! $dom_installed,
			'recommendation' => esc_html__('You must enable DOM extension (Document Object Model) in PHP. It\'s used for HTML processing. Contact the support service of your hosting provider. They know what to do.', 'deblocker' )
		];
	}
	...
	public function get_xml_installed() {
		...
		return [
			'value' => $xml_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
			'warning' => ! $xml_installed,
			'recommendation' => esc_html__('You must enable XML extension in PHP. It\'s used for XML processing. Contact the support service of your hosting provider. They know what to do.', 'deblocker' )
		];
	}
	...
	public function get_allow_url_fopen() {
		...
		return [
			'value' => $allow_url_fopen ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
			'warning' => ! $allow_url_fopen,
			'recommendation' => esc_html__('You need to enable allow_url_fopen directive in PHP. It\'s used for download updates and other data from our server. Contact the support service of your hosting provider. They know what to do.', 'deblocker' )
		];
	}
	...
	public function get_bcmath_installed() {
		...
		return [
			'value' => $bcmath_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'deblocker') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'deblocker' ),
			'warning' => ! $bcmath_installed,
			'recommendation' => esc_html__('You must enable BCMath extension (Arbitrary Precision Mathematics) in PHP. Contact the support service of your hosting provider. They know what to do.', 'deblocker' )
		];
	}
	...
	public function get_write_permissions() {
		...
		$paths_to_check = [
			ABSPATH => esc_html__('WordPress root directory', 'deblocker' )
		];
		...
		if ( $wp_upload_dir['error'] ) {
			$write_problems[] = esc_html__('WordPress root uploads directory', 'deblocker' );
		}

		$deblocker_uploads_path = $wp_upload_dir['basedir'] . '/deblocker';

		if ( is_dir( $deblocker_uploads_path ) ) {
			$paths_to_check[ $deblocker_uploads_path ] = esc_html__('DeBlocker uploads directory', 'deblocker' );
		}

		$paths_to_check[ WP_PLUGIN_DIR ] = esc_html__('Plugins directory', 'deblocker' );

		$htaccess_file = ABSPATH . '/.htaccess';

		if ( file_exists( $htaccess_file ) ) {
			$paths_to_check[ $htaccess_file ] = esc_html__('.htaccess file', 'deblocker' );
		}
		...
		if ( $write_problems ) {
			$value = '<i class="material-icons mdc-system-no">error</i>' . esc_html__('There are some writing permissions issues with the following directories/files:', 'deblocker' ) . "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ";

			$value .= implode( "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ", $write_problems );
		} else {
			$value = '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('All right', 'deblocker' );
		}
