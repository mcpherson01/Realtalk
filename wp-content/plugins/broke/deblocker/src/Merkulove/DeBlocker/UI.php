...
final class UI {
	...
	**/
                    ...
                    <ul class="mdc-list mdc-select-width">
                        <?php foreach ( $options as $key => $value ) : ?>
                            ...
                            <li class="mdc-list-item <?php esc_attr_e( $selected_class ); ?>" data-value="<?php esc_attr_e( $key ); ?>" role='option'>
                                <?php $img_file = DeBlocker::$path . 'images/' . $key . '.svg'; ?>
                                <?php if ( file_exists( $img_file ) ) : ?>
                                    <img src="<?php echo esc_url( DeBlocker::$url . "images/" . $key . ".svg" ); ?>" alt="<?php echo esc_attr( $key ); ?>" >
                                <?php endif; ?>
	...
	public function render_icon( $value, $label = '', $helper_text = '', $attributes = [], $meta = [] ) {
		...
		/** Get icon folder URL. */
		$icon_folder_url = DeBlocker::$url . 'images/mdc-icons/';
		...
		foreach ( $meta as $meta_json ) {
			...
			if ( ! (
				...
				echo '<div class="mdp-alert mdp-alert-warning">'. esc_html__( 'Error: Icon library not found.', 'deblocker' ) . '</div>';
				return;
			}
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'deblocker' ); ?></button>
                <button class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload SVG', 'deblocker' ); ?></button>
            </div>
