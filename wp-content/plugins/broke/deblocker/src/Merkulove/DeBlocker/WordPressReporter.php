...
final class WordPressReporter {
	...
	public function get_fields() {
		return [
			'version'               => esc_html__( 'Version', 'deblocker' ),
			'site_url'              => esc_html__( 'Site URL', 'deblocker' ),
			'home_url'              => esc_html__( 'Home URL', 'deblocker' ),
			'home_path'             => esc_html__( 'Home Path', 'deblocker' ),
			'plugin_path'           => esc_html__( 'Plugin Path', 'deblocker' ),
			'is_multisite'          => esc_html__( 'WP Multisite', 'deblocker' ),
			'max_upload_size'       => esc_html__( 'Max Upload Size', 'deblocker' ),
			'memory_limit'          => esc_html__( 'Memory limit', 'deblocker' ),
			'permalink_structure'   => esc_html__( 'Permalink Structure', 'deblocker' ),
			'language'              => esc_html__( 'Language', 'deblocker' ),
			'timezone'              => esc_html__( 'Timezone', 'deblocker' ),
			'admin_email'           => esc_html__( 'Admin Email', 'deblocker' ),
			'debug_mode'            => esc_html__( 'Debug Mode', 'deblocker' ),
		];
	}
	...
	public function get_memory_limit() {
		...
		if ( $memory_limit_bytes < $min_recommended_bytes ) {
			$result['recommendation'] = esc_html__( 'We recommend setting memory to at least 64M. For more information, ask your hosting provider.','deblocker' );
	...
	public function get_is_multisite() {
		return [
			'value' => is_multisite() ? esc_html__( 'Yes', 'deblocker' ) : esc_html__( 'No', 'deblocker' )
		];
	}
	...
	public function get_permalink_structure() {
		...
		if ( ! $structure ) {
			$structure = esc_html__( 'Plain', 'deblocker' );
		}
	...
	public function get_debug_mode() {
		return [
			'value' => WP_DEBUG ? esc_html__('Active', 'deblocker' ) : esc_html__('Inactive', 'deblocker' )
		];
	}
