...
/**
 * Check Schema enabled entries, used by Locations targeting annd
...
function schema_premium_check_location_target_match( $post_id = null, $property = '', $value = '' ) {
	...
	foreach ( $location_target_match as $target_match ) {
		...
		if ( isset($target_match['match']) && $target_match['match'] == 1 ) {
			...
			$schema 	= schema_wp_get_default_schemas($target_match['schema_type']);
			$properties = schema_wp_get_properties_markup_output( $post_id, $schema['properties'] );
			
			// Debug
			//echo $target_match['schema_type'];
			//echo'<pre>';print_r($properties);echo'</pre>';
			
			if ( isset($properties[$property]) && $properties[$property] == $value ) {
				return true; 
			}
		}
	}
