...
if (!class_exists('A2W_Woocommerce')) {
    ...
    class A2W_Woocommerce {
        ...
        public function add_product($product, $params = array()) {
            ...
            if (!a2w_check_defined('A2W_DO_NOT_IMPORT_VARIATIONS') && $product_type !== "external" && !empty($product['sku_products']['variations']) && count($product['sku_products']['variations']) > 1) {
        ...
        public function upd_product($product_id, $product, $params = array()) {
            ...
            // update global stock
            if (get_option('woocommerce_manage_stock', 'no') === 'yes') {
                if ($total_quantity>0 || $on_not_available_product === 'zero') {
                ...
                if ($on_not_available_product === 'trash') {
                    ...
                    foreach ($wc_product->get_children() as $var_id) {
                        if($on_not_available_variation === "trash"){
                            ...
                            $var->delete(true);
                        } else if($on_not_available_variation === "zero" && !$product['disable_var_quantity_change']){
                            $var = wc_get_product($var_id);
        ...
        private function add_variation($product_id, $product, $is_update = false) {
                ...
                if (!$old_vid && (!$is_update || $on_new_variation_appearance === 'add')) {  
                    ...
                    foreach ($used_variation_attributes as $attr_tax => $v) {
                        ...
                        if ($attr_value = get_post_meta($variation_id, $tmp_attr_name, true)) {
                            ...
                            if (!isset($original_variations_attributes[$tmp_name])) {
                                $original_variations_attributes[$tmp_name] = array('original_attribute_id' => $v['original_attribute_id'], 'current_name' => urldecode($tmp_name), 'name' => urldecode(!empty($attr['original_name']) ? $attr['original_name'] : $attr['name']), 'values' => array());
                            } else {
            ...
            foreach ($old_variations as $variation_id) {
                ...
                } else if ($on_not_available_variation === 'zero') {
                    $var_product = wc_get_product($variation_id);
