...
class Wpw_Auto_Poster_BA_Posting {
	...
	public function wpw_auto_poster_post_to_bufferapp( $post ) {
		...
		if( isset( $_SESSION['wpw_auto_poster_ba_access_token'] ) && !empty( $_SESSION['wpw_auto_poster_ba_access_token'] ) ) { 
			...
			$trim_content = strip_shortcodes( $post->post_content );
			...
			$replace_arr = array( $title, $postlink, $first_name, $last_name, get_option( 'blogname'), $excerpt, $hashtags, $hashcats, $trim_content);
			...
			//$title = apply_filters('wpw_auto_poster_buffer_posting_title', $title, $description );
