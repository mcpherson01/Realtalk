...
if ($values['fields'] and $page_num > 0) {
    if($field_page_break_top_bar != 1) {
        ...
        if ($field_page_break_type == 'survey' && $field_page_break_type_possition=='bottom') {
            ...
            $total_page_shows = $page_num;
            if($field_page_break_top_bar != 1) {
                $arf_form .= '<div style="clear:both; margin-top:25px;"></div><div class="arf_survey_nav"><div id="current_survey_page" class="survey_step">' . __('Step', 'ARForms') . ' </div><div id="current_survey_page" class="current_survey_page">1</div><div class="survey_middle">' . __('of', 'ARForms') . '</div><div id="total_survey_page" class="total_survey_page">' . ($total_page_shows + 1) . '</div></div>';
	...
	if($doliact == 0)
	{
	  $mybrand = 0;
        ...
        if ($mybrand == 0) {
            ...
            $arf_form .='<div id="brand-div" class="brand-div ' . $_SESSION['label_position'] . '_container" style="margin-top:30px; font-size:12px !important; color: #444444 !important; display:block !important; visibility: visible !important;">' . addslashes(__('Powered by', 'ARForms')) . '&#32;';
            if(is_ssl()) {
