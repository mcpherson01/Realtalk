...
if ( ! class_exists( 'CP_Addon_Loader' ) ) {
	...
	final class CP_Addon_Loader {
		 ...
		 * @since 1.2.2
		 */
		function cp_pro_mautic_notice() {

			$live_popups = cp_get_live_popups();
			$flag        = 0;
			foreach ( $live_popups as $key => $style_id ) {
			...
			if ( 1 == $flag ) {
				$message = __( 'Looks like you have used a Mautic API integration in Convert Pro. We recommend you to please integrate it again using the Username and Password method.<a href="https://www.convertpro.net/new-integration-method-with-mautic/" target="_blank">Read more</a>.', 'convertpro-addon' );
