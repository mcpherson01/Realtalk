...
class FES_vdw_gallery_id_Field extends FES_Field {
	...
	/** @var array Characteristics are things that can change from field to field of the same field type. Like the placeholder between two vdw_gallery_id fields. Stored in db. */
	...
	public function set_title() {
		$title = _x( 'Gallery Images', 'FES Field title translation', 'mayosis' );
	...
	public function render_field_admin( $user_id = -2, $readonly = -2 ) {
		...
		$user_id   = apply_filters( 'fes_render_vdw_gallery_id_field_user_id_admin', $user_id, $this->id );
		$readonly  = apply_filters( 'fes_render_vdw_gallery_id_field_readonly_admin', $readonly, $user_id, $this->id );
		...
		ob_start(); ?>
			<div class="fes-fields">
				 <table class="<?php echo sanitize_key( $this->name() ); ?>">
					<thead>
						<tr>
							<td class="fes-file-column" colspan="2"><?php _e( 'Image URL', 'edd_fes' ); ?></td>
							...
							foreach ( $uploaded_items as $index => $attach_id ) {
								...
								<tr class="fes-single-variation">
									 ...
									 <td class="fes-url-choose-row" width="1%">
												<a href="#" class="edd-submit button upload_file_button" data-choose="<?php _e( 'Choose Image', 'edd_fes' ); ?>" data-update="<?php _e( 'Insert image URL', 'edd_fes' ); ?>">
												<?php echo str_replace( ' ', '&nbsp;', __( 'Choose Image', 'edd_fes' ) ); ?></a>
									 </td>
									 <?php if ( empty( $this->characteristics['single'] ) || $this->characteristics['single'] !== 'yes' ) { ?>
					...
					<tfoot>
						<tr>
							<th colspan="5">
								<a href="#" class="edd-submit button insert-file-row" id="<?php echo sanitize_key( $this->name() ); ?>"><?php _e( 'Add Image', 'edd_fes' ); ?></a>
							</th>
						</tr>
					</tfoot>
					<?php endif; ?>
	...
	public function render_field_frontend( $user_id = -2, $readonly = -2 ) {
		...
		$user_id   = apply_filters( 'fes_render_vdw_gallery_id_field_user_id_frontend', $user_id, $this->id );
		$readonly  = apply_filters( 'fes_render_vdw_gallery_id_field_readonly_frontend', $readonly, $user_id, $this->id );
		...
		ob_start(); ?>
			<div class="fes-fields">
				 <table class="<?php echo sanitize_key( $this->name() ); ?>">
					<thead>
						<tr>
							<th class="fes-file-column" colspan="2"><?php _e( 'Image URL', 'edd_fes' ); ?></th>
							<?php if ( empty( $this->characteristics['single'] ) || $this->characteristics['single'] !== 'yes' ) { ?>
					...
					<tbody class="fes-variations-list-<?php echo sanitize_key( $this->name() ); ?>">
							...
							foreach ( $uploaded_items as $index => $attach_id ) {
								$download = wp_get_attachment_url( $attach_id ); ?>
								<tr class="fes-single-variation">
									 <td class="fes-url-row fes--my--url-row">
									 ...
									 <td class="fes-url-choose-row fes--my--btn-row">
												<a href="#" class="edd-submit button upload_file_button" data-choose="<?php _e( 'Choose Image', 'edd_fes' ); ?>" data-update="<?php _e( 'Insert image URL', 'edd_fes' ); ?>">
												<?php echo str_replace( ' ', '&nbsp;', __( 'Choose Image', 'edd_fes' ) ); ?></a>
									 </td>
									 <?php if ( empty( $this->characteristics['single'] ) || $this->characteristics['single'] !== 'yes' ) { ?>
					...
					<tfoot>
						<tr>
							<th colspan="5">
								<a href="#" class="fes--my--submit edd-submit button insert-file-row" id="<?php echo sanitize_key( $this->name() ); ?>"><?php _e( 'Add Image', 'edd_fes' ); ?></a>
							</th>
						</tr>
					</tfoot>
					<?php endif; ?>
	...
	public function get_field_value_admin( $save_id = -2, $user_id = -2, $public = -2 ) {
		...
		if ( $this->type === 'user' ) {
			...
			$value = apply_filters( 'fes_get_custom_vdw_gallery_id_value_admin', $save_id, $user_id, $public );
		}
	...
	public function get_field_value_frontend( $save_id = -2, $user_id = -2, $public = -2 ) {
		...
		if ( $this->type === 'user' ) {
			...
			$value = apply_filters( 'fes_get_custom_vdw_gallery_id_value_frontend', $save_id, $user_id, $public );
		}
