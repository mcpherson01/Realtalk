...
function find_min_shipping_price(items, default_method) {
    ...
    jQuery.each(items, function (i, item) {
        if (p < 0 || item.freightAmount.value < p || item.serviceName==default_method) {
            p = item.freightAmount.value;
            result = {'price': item.freightAmount.value, 'formated_price': item.freightAmount.value > 0.009 ? (item.localPriceFormatStr || item.priceFormatStr) : 'Free', 'name': item.company, 'time': item.time};
...
(function ($, window, document, undefined) {
    $(function () {
        ...
        function prepareProductUpdateData(product, updateDescription = false){
            var updateData = {title: $(product).find('.title').val(), tags: $(product).find('.tags').val(), type: $(product).find('.type').val(), status: $(product).find('.status').val(), categories: $(product).find('.categories').val(), attr_names: []};
            ...
            a2w_update_product($(product).attr('data-id'), {variations}, response=>{
                if(response.state === "ok" && response.new_attr_mapping){
                    $.each(response.new_attr_mapping, function (i, a) {
                        $('input[data-id="'+a.old_attr_id+'"]').attr('data-id',a.new_attr_id);
                    });
                }
            });
        });
