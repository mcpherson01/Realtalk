...
class Thrive_Dash_Api_Ontraport {
	...
	public function v2Call( $path, $params = array(), $method = 'GET' ) {
		...
		if ( $status != 200 && $status != 204 ) {
			throw new Thrive_Dash_Api_Ontraport_Exception( 'Call failed: ' . ( empty( $response['body'] ) ? 'HTTP status code ' . $status
					: $response['body'] ) );
		}
		...
		if ( ! empty( $data['code'] ) ) {
			throw new Thrive_Dash_Api_Ontraport_Exception( 'API Error: ' . isset( $data['result_message'] ) ? $data['result_message']
				: (int) $data['result_code'] );
		}
