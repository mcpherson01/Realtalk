	...
	if( function_exists('acf_add_local_field_group') ):
		...
		*/
		if ( !empty($schema_default) && !empty($schema_default['subtypes']) ) {
			...
			// Add General sub type for Article
			if ($schema_type == 'Article') 
				$subtype_choices = array_merge($subtype_choices, array ('General' => 'General'));
		...
		/*
		acf_add_local_field( apply_filters( 'schema_wp_info_post_meta_options', array(
			'key' => 'field_schema_learn_link',
			'name' => 'schema_learn_link',
			'label' => __('Learn', 'schema-premium'),
			'instructions'	=> '<a target="_blank" href="https://schema.org/'.$schema_type.'">https://schema.org/'.$schema_type.'</a>', 
			'type' => 'message',
			'parent' => 'schema_group',
			'wrapper' => array (
				'width' => '50',
		...
		*/
		...
		if ( isset($schema_subtype) && '' != $schema_subtype ) {
			...
			$schema_type_properties		= is_object($schema_class) ? $schema_class->properties() : $enabled['properties'];
		}
		...
		if (! empty($schema_type_properties) ) {
			...
			acf_add_local_field_group(array (
				...
				'title' => $schema_type . ' ' . __('Properties', 'schema-premium'),
				'location' => array (
					array (
			...
			foreach ( $schema_type_properties as $property => $property_details) { 
				...
				if ( $properties_instructions_enable != 'yes' && $property_details['field_type'] == 'message' ) {
					continue; // get oouut here... disable messages if instructions is disabled 
				};
...
function schema_wp_do_acf_properties_fields( $post_id, $property, $property_details, $sub_field = false ) {
	...
	switch ($property) {
		...
		case 'priceRange':
			...
			break;
		case 'Thing_properties_info':
		case 'CreativeWork_properties_info':
		case 'Article_properties_info':
			$message = true;
			break;
		default:
