...
if ( ! class_exists('Schema_WP_CPT_columns') ) {
	...
	class Schema_WP_CPT_columns{
		...
		function do_column($post_id,$column,$column_name){
			if (in_array($column['type'],array('post_id','text','thumb','post_meta','custom_tax','post_meta_array','indicator','cpt_post_count','markup_source', 'rating_stars') ) )
				echo $column['prefix'];
			switch ($column['type']) {
				case 'post_id':
					echo $post_id;
				...
				case 'rating_author':
					...
					break;
				case 'rated_post_title':
					$rated_post_id = get_post_meta( $post_id, $column['meta_key'], true );
					if ( isset($rated_post_id) && $rated_post_id) {
						//echo get_the_title($rated_post_id);
						echo '<a href="' . get_permalink($rated_post_id) . '">' . get_the_title($rated_post_id) . '</a>';
					}
					break;
				case 'attachment_id':
