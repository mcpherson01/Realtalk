			...
			//if ( isset($enabled['match']) ) {
				...
				if ( is_array ($schema_type_properties) && !empty($schema_type_properties) ) {	
					...
					if ( $match_count > 1 ) { // Add tabs only if there is more than one schema type enabled
						...
						acf_add_local_field(array(
							...
							'label' 	=> isset($enabled['schema_subtype']) && '' != $enabled['schema_subtype'] ? $enabled['schema_subtype'] : $schema_default['lable'],
				...
				if ( is_edit_page('new') && $post_id == 0 && !$enabled['match']
					...
					acf_add_local_field( array(
						...
						'label' => '<span style="color:#999;">' . __('Not available?', 'schema-premium') . '</span>',
						'instructions'	=> '', 
						'message'	=> '<span class="dashicons dashicons-info" style="color:#999;"></span> ' . __('Properties maybe not be available before saving the post! (If you are using Gutenberg editor, save and refresh the page)', 'schema-premium'), 
						'type' => 'message',
						'parent' => 'group_schema_properties'
					) );
				}
