	...
	class Schema_WP_Product {
		...
		public function properties() {
			...
			$properties = array (
					...
					'price' => array(
						...
						'field_type' 	=> 'text',
						'markup_value' => 'new_custom_field',
						'instructions' 	=> __('The offer price of a product. (this also is the lowPrice of AggregateOffer)', 'schema-premium'),
						'required' 		=> true
					),
					'highPrice' => array(
		...
		*/
		public function schema_output( $post_id = null ) {
			...
			// AggregateOffer
			// @siince 1.0.3
			if ( isset($properties['highPrice']) ) {
				$schema['offers']['@type'] = 'AggregateOffer';
				$schema['offers']['lowPrice'] = isset($properties['price']) ? $properties['price'] : '';
				$schema['offers']['highPrice'] = $properties['highPrice'];
				$schema['offers']['offerCount'] = isset($properties['offerCount']) ? $properties['offerCount'] : '';
				unset($schema['offers']['price']); // unset price
			}
