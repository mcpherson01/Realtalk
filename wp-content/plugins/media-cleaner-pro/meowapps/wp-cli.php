...
class MeowAppsPro_WPMC_WP_CLI extends WP_CLI_Command {
	...
	public function scan( $args ) {
		...
		// Content Check + Live Check
		if ( $check_content === 'yes' ) {
			$progress = \WP_CLI\Utils\make_progress_bar( 'Analyze content:   ', 100 );
			$finished = false;
			$limit = 0;
			$limitSize = 1000;
			while ( !$finished ) {
				$finished = $wpmc->engine->parse( $limit, $limitSize, $message );
		...
		if ( $method === 'Filesystem' ) {
			$progress = \WP_CLI\Utils\make_progress_bar( 'Read Filesystem:   ', 100 );
			$dirs = array( '.' );
			while ( count( $dirs ) > 0 ) {
				$dir = array_pop( $dirs );
