...
/*
    echo "<pre>";
    print_r(get_option("njt_bulksender_active"));
    echo "</pre>";
*/
//delete_option("njt_bulksender_active");
$check_active = -1;
if(get_option("njt_bulksender_active")){
    $bulksender_active = get_option("njt_bulksender_active");
    $day_current = date('Y-m-d H:i:s',time());
    $supported_until = $bulksender_active["supported_until"];
