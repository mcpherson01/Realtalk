...
final class PluginUpdater {
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-deblocker/deblocker.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	public function update_plugins( $update_plugins ) {
		...
		$plugin_info = get_transient( 'mdp_deblocker_plugin_info' );
		...
		if ( false === $plugin_info ) {
			...
			/** Set cache for 5 min. */
			set_transient( 'mdp_deblocker_plugin_info', $plugin_info, 300 ); // 5 min cache.
		}
		...
		$current_version = DeBlocker::get_instance()->get_version();
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['deblocker/deblocker.php'] = (object)[
				'slug'          => 'deblocker',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'deblocker' !== $args->slug ) { return false; }
		
		/** Trying to get from cache first. */
		$remote = get_transient( 'mdp_deblocker_upgrade' );
		...
		if ( false === $remote ) {
			...
			if (
				...
				set_transient( 'mdp_deblocker_upgrade', $remote, 3600 ); // 1 hour cache.
			}
	...
	public function reset_cache() {
		delete_transient( 'mdp_deblocker_upgrade' );
		delete_transient( 'mdp_deblocker_plugin_info' );
	}
	...
	public function show_plugin_update_message() {
		...
		if ( ! PluginActivation::get_instance()->is_activated() ) {
			...
			esc_attr_e( 'Please visit the plugin page on the Envato market to ', 'deblocker' );
			echo ' <a href="' . EnvatoItem::get_instance()->get_url() . '" target="_blank">';
			esc_attr_e( 'download ', 'deblocker' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'deblocker' );
		}
