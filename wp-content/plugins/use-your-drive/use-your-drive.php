 ...
 */
/* * ***********SYSTEM SETTINGS****************** */
define('USEYOURDRIVE_VERSION', '1.14.6');
...
class Main {
    ...
    public function __construct() {
        ...
        add_filter('mce_css', array(&$this, 'enqueue_tinymce_css_frontend'));
        ...
        add_action('useyourdrive_lost_authorisation_notification', array(&$this, 'send_lost_authorisation_notification'), 10, 1);
        ...
        add_action('wp_ajax_nopriv_useyourdrive-move-entries', array(&$this, 'start_process'));
        add_action('wp_ajax_useyourdrive-move-entries', array(&$this, 'start_process'));
        ...
        add_action('wp_ajax_nopriv_useyourdrive-edit', array(&$this, 'start_process'));
        add_action('wp_ajax_useyourdrive-edit', array(&$this, 'start_process'));
        ...
        add_action('wp_ajax_nopriv_useyourdrive-check-recaptcha', array(&$this, 'check_recaptcha'));
        add_action('wp_ajax_useyourdrive-check-recaptcha', array(&$this, 'check_recaptcha'));
        ...
        add_action('wp_ajax_useyourdrive-reset-statistics', array(&$this, 'start_process'));
...
%filelist%
  ...
  <td style="font-weight: bold;">%file_size%</td>
        ...
        if ($this->settings['shortlinks'] === 'Google') {
            $this->settings['shortlinks'] = 'None';
            $updated = true;
        }

        if ($this->settings['mediaplayer_skin'] === 'default') {
            $this->settings['mediaplayer_skin'] = 'Default_Skin';
            $updated = true;
        }
    ...
    public function load_scripts() {
        ...
        if ($this->settings['recaptcha_sitekey'] !== '') {
            ...
            wp_register_script('google-recaptcha', $url, array(), '3.0', true);
        }
        ...
        wp_register_script('WPCloudplugin.Libraries', plugins_url('includes/js/library.js', __FILE__), array('WPCloudPlugins.Polyfill', 'jquery'), USEYOURDRIVE_VERSION, true);
        ...
        wp_register_script('UseyourDrive.tinymce', plugins_url('includes/js/Tinymce_popup.js', __FILE__), array('jquery-ui-accordion', 'jquery'), USEYOURDRIVE_VERSION, true);
        ...
        $localize = array(
            ...
            'str_move_title' => __('Move', 'useyourdrive'),
            'str_copy_title' => __('Copy', 'useyourdrive'),
            'str_save_title' => __('Save', 'useyourdrive'),
            'str_zip_title' => __('Create zip file', 'useyourdrive'),
            'str_account_title' => __('Select account', 'useyourdrive'),
    ...
    public function load_styles() {
        ...
        wp_register_style('UseyourDrive', plugins_url("css/main$is_rtl_css.css", __FILE__), array(), USEYOURDRIVE_VERSION);
        wp_register_style('UseyourDrive.tinymce', plugins_url("css/tinymce$is_rtl_css.css", __FILE__), array(), USEYOURDRIVE_VERSION);
        ...
        if ($this->settings['always_load_scripts'] === 'Yes') {
            ...
            if (defined('WPCP_DISABLE_FONTAWESOME') === false) {
                ...
                if ($this->settings['fontawesomev4_shim'] === 'Yes') {
    ...
    public function check_recaptcha() {
        ...
        if ($resp->isSuccess()) {
            ...
            echo json_encode(array('verified' => true, 'msg' => $resp->getErrorCodes()));
        }
    ...
    public function send_lost_authorisation_notification($account_id = null) {
        ...
        $subject = get_bloginfo() . ' | ' . __('ACTION REQUIRED: WP Cloud Plugin lost authorization to Google Drive account', 'useyourdrive') . ':' . (!empty($account) ? $account->get_email() : '');
    ...
    public function synchronize_cache() {
        ...
        foreach ($this->get_accounts()->list_accounts() as $account_id => $account) {
            if ($account->get_authorization()->has_access_token() === false) {
                error_log("[Use-your-Drive message]: WP Cron cannot be run for {$account->get_email()} without access to the cloud");
                continue;
            }
    ...
    public function load_shortcode_buttons() {
        ...
        add_filter("mce_external_plugins", array(&$this, "register_tinymce_plugin"), 999);

        /* Add a callback to add our button to the TinyMCE toolbar */
        add_filter('mce_buttons', array(&$this, 'register_tinymce_plugin_buttons'), 999);
    ...
    function enqueue_tinymce_css_frontend($mce_css) {
        if (!empty($mce_css)) {
            $mce_css .= ',';
        }
...
function UseyourDrive_Deactivate() {
    ...
    foreach ($UseyourDrive->get_accounts()->list_accounts() as $account_id => $account) {
        if (($timestamp = wp_next_scheduled('useyourdrive_lost_authorisation_notification', array('account_id' => $account_id))) !== false) {
            wp_unschedule_event($timestamp, 'useyourdrive_lost_authorisation_notification', array('account_id' => $account_id));
        }
    }
