...
class Upload {
        ...
        foreach ($response['files'] as &$file) {
            ...
            try {
                if ($current_entry_id === false) {
                    $googledrive_file->setParents(array($upload_folder_id));
                    $request = $this->get_app()->get_drive()->files->create($googledrive_file, array("userIp" => $this->get_processor()->get_user_ip(), 'supportsAllDrives' => true));
                } else {
                    $request = $this->get_app()->get_drive()->files->update($current_entry_id, $googledrive_file, array("userIp" => $this->get_processor()->get_user_ip(), 'supportsAllDrives' => true));
                }
            } catch (\Exception $ex) {
            ...
            $api_entry = $this->get_app()->get_drive()->files->get($upload_status->getId(), array("userIp" => $this->get_processor()->get_user_ip(), 'fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true));

            if (($api_entry->getSize() === 0) && (strpos($api_entry->getMimetype(), 'google-apps') === false)) {
                $deletedentry = $this->get_app()->get_drive()->files->delete($api_entry->getId(), array("userIp" => $this->get_processor()->get_user_ip(), 'supportsAllDrives' => true));
    ...
    public function do_upload_direct() {
        ...
        if (empty($current_entry_id)) {
            $googledrive_file->setParents(array($upload_folder_id));
            $request = $this->get_app()->get_drive()->files->create($googledrive_file, array("userIp" => $this->get_processor()->get_user_ip(), 'fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true));
        } else {
            $request = $this->get_app()->get_drive()->files->update($current_entry_id, $googledrive_file, array("userIp" => $this->get_processor()->get_user_ip(), 'fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true));
        }
    ...
    public function upload_convert() {
        ...
        try {
            ...
            $api_entry = $this->get_app()->get_drive()->files->copy($cachedentry->get_id(), $entry, array("userIp" => $this->get_processor()->get_user_ip(), 'fields' => $this->get_client()->apifilefields, 'supportsAllDrives' => true));

            if ($api_entry !== false && $api_entry !== null) {
                $new_id = $api_entry->getId();
                /* Remove file from Cache */
                $deleted_entry = $this->get_app()->get_drive()->files->delete($cachedentry->get_id(), array("userIp" => $this->get_processor()->get_user_ip(), 'supportsAllDrives' => true));
    ...
    public function create_folder_structure($path) {
        ...
        foreach ($folders as $key => $name) {
            ...
            $cached_entry = $this->get_processor()->get_cache()->get_node_by_name($name, $current_folder['folder']);
            ...
            if ($cached_entry) {
                ...
                $this->get_processor()->get_cache()->pull_for_changes(true, -1);
            ...
            try {
                ...
                $newfolder->setParents(array($current_folder_id));
                $api_entry = $this->get_app()->get_drive()->files->create($newfolder, array("fields" => $this->get_client()->apifilefields, "supportsAllDrives" => true, "userIp" => $this->get_processor()->get_user_ip()));
