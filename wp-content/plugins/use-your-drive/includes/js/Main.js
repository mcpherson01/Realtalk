  ...
  $.widget("cp.UseyourDrive", {
    ...
    _initUploadBox: function () {
      ...
      upload_box.fileupload({
        ...
        if ($('.UseyourDrive[uploading=true]').length > 0) {
          return;
        }
    ...
    _initEditMenu: function () {
      ...
      self.element.find('.entry_action_download').unbind('click').click(function (e) {
        e.stopPropagation();
        return self._actionDownloadEntry($(this));
      });

      /* Save As Event */
      self.element.find('.entry_action_export').unbind('click').click(function (e) {
        e.stopPropagation();
        return self._actionExportEntry(e, $(this));
      });
      ...
      self.element.find('.entry_action_delete').unbind('click').click(function (e) {
        ...
        self.element.find(".entry[data-id='" + dataid + "'] .selected-files:checkbox").prop("checked", true);
        var entries = self.element.find("input[name='selected-files[]']:checked");
        self._actionDeleteEntries(entries);
      });
    ...
    renderBreadCrumb: function () {
      ...
      $breadcrumb_element.asBreadcrumbs({
        ...
        dropdownMenuClass: "uyd-dropdown-menu",
      ...
      $breadcrumb_element.find('.uyd-dropdown-menu li').click(function () {
        $breadcrumb_element.find('li a.folder[href="' + $(this).find('a').attr("href") + '"]').trigger('click');
        $breadcrumb_element.find('.uyd-dropdown').removeClass('open');
      });

      $breadcrumb_element.find('.uyd-toggle').click(function () {
        $breadcrumb_element.find('.uyd-dropdown').addClass('open');
      });

      $(document).mouseup(function (e) {
        var container = $breadcrumb_element.find('.uyd-dropdown-menu');
    ...
    _pipeline: function (opts) {
      ...
      return function (request, drawCallback, settings) {
        ...
        $.extend(request, d);
        var storage_id = (request.listtoken + request._ajax_nonce + (typeof request.account_id === 'undefined' ? '' : request.account_id) + request.filelayout + (typeof request.id === 'undefined' ? request.lastFolder : request.id) + request.sort + request.query);
