...
class Client {
    ...
    public function get_account_info() {
        $accountInfo = $this->get_app()->get_user()->userinfo->get();
        return $accountInfo;
    }
    ...
    public function get_drive_info() {
        $driveInfo = $this->get_app()->get_drive()->about->get(array("fields" => 'importFormats,kind,storageQuota,user'));
        return $driveInfo;
    }
    ...
    public function get_multiple_entries($entries) {
        ...
        if (count($entries) === 1) {
            $api_entry = $this->get_app()->get_drive()->files->get(reset($entries), array("userIp" => $this->_user_ip, "supportsAllDrives" => $this->useteamfolders, 'fields' => $this->apifilefields));
        ...
        foreach ($entries as $entryid) {
            $batch->add($this->get_app()->get_drive()->files->get($entryid, array("fields" => $this->apifilefields, "supportsAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip)), $entryid);
        }
    ...
    public function get_entry($entryid = false, $checkauthorized = true) {
        ...
        if (!$cachedentry) {
            ...
            try {
                $api_entry = $this->get_app()->get_drive()->files->get($entryid, array("userIp" => $this->_user_ip, "supportsAllDrives" => $this->useteamfolders, 'fields' => $this->apifilefields));
    ...
    public function update_expired_entry(CacheNode $cachedentry) {
        ...
        try {
            $api_entry = $this->get_app()->get_drive()->files->get($entry->get_id(), array("userIp" => $this->_user_ip, "supportsAllDrives" => $this->useteamfolders, 'fields' => $this->apifilefieldsexpire));
    ...
    public function update_expired_folder(CacheNode $cachedentry) {
        ...
        $params = array('q' => "'" . $entry->get_id() . "' in parents and mimeType != 'application/vnd.google-apps.folder' and trashed = false", "fields" => $this->apilistfilesexpirefields, "pageSize" => 999, "supportsAllDrives" => $this->useteamfolders, "includeItemsFromAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip);
    ...
    public function get_root_folder() {
        ...
        if ($root_node !== false && $root_node->get_entry() !== null) {
            return $root_node;
        }
    ...
    public function get_folder($folderid = false, $checkauthorized = true) {
        ...
        if (!$cachedfolder) {
            $params = array('q' => "'" . $folderid . "' in parents and trashed = false", "fields" => $this->apilistfilesfields, "pageSize" => 999, "supportsAllDrives" => $this->useteamfolders, "includeItemsFromAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip);

            $this->get_library()->setUseBatch(true);
            $batch = new \UYDGoogle_Http_Batch($this->get_library());

            $batch->add($this->get_app()->get_drive()->files->get($folderid, array("fields" => $this->apifilefields, "supportsAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip)), 'folder');
            ...
            if ($folder instanceof \Exception) {
                error_log('[Use-your-Drive message]: ' . sprintf('Google API Error (Folder does not exist on the Google Drive) on line %s: %s', __LINE__, $folder->getMessage()));
                return false;
            }
    ...
    public function get_shared_contents($checkauthorized = true, $isupdating = false) {
        ...
        if (!$cachedfolder) {
            $params = array('q' => "sharedWithMe = true and trashed = false", "fields" => $this->apilistfilesfields, "pageSize" => 999, "supportsAllDrives" => $this->useteamfolders, "includeItemsFromAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip);
    ...
    public function delete_entries($entries_to_delete = array()) {
        ...
        foreach ($entries_to_delete as $target_entry_path) {
            ...
            if (
                    $this->get_processor()->get_shortcode_option('deletetotrash') === '1' ||
                    ($target_entry->get_permission('candelete') === false && $target_entry->get_permission('cantrash') === true)
            ) {
                $updateentry = new \UYDGoogle_Service_Drive_DriveFile();
                $updateentry->setTrashed(true);
                $batch->add($this->get_app()->get_drive()->files->update($target_entry->get_id(), $updateentry, array("supportsAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip)), $target_entry->get_id());
            } else {
                $batch->add($this->get_app()->get_drive()->files->delete($target_entry->get_id(), array("supportsAllDrives" => $this->useteamfolders, "userIp" => $this->_user_ip)), $target_entry->get_id());
            }
        }
        ...
        foreach ($batch_result as $entry_id => $api_entry) {
            do_action('useyourdrive_log_event', 'useyourdrive_deleted_entry', $deleted_entries[$api_entry->getId()], array('to_trash' => $this->get_processor()->get_shortcode_option('deletetotrash')));
            $this->get_cache()->remove_from_cache($api_entry->getId(), 'deleted');
        }
    ...
    function move_entries($entries = array(), $target, $copy = false) {
        ...
        if ($cached_target === false) {
            error_log('[Use-your-Drive message]: ' . sprintf('Failed to move as target folder %s is not found.', $target_entry->get_path()));
        ...
        foreach ($entries as $entry_id) {
            ...
            if (($entry->is_dir()) && ($this->get_processor()->get_user()->can_move_folders() === false)) {
                error_log('[Use-your-Drive message]: ' . sprintf('Failed to move %s as user is not allowed to move folders.', $target_entry->get_path()));
                $entries_to_move[$cached_entry->get_id()] = false;
                continue;
            }

            if (($entry->is_file()) && ($this->get_processor()->get_user()->can_move_files() === false)) {
                error_log('[Use-your-Drive message]: ' . sprintf('Failed to move %s as user is not allowed to remove files.', $target_entry->get_path()));
                $entries_to_move[$cached_entry->get_id()] = false;
                continue;
            }
            ...
            if (!$cached_entry->is_in_folder($cached_current_folder->get_id())) {
                error_log('[Use-your-Drive message]: ' . sprintf('Failed to move %s as user is not allowed to move items in this directoty.', $target_entry->get_path()));
                $entries_to_move[$cached_entry->get_id()] = false;
                continue;
            }
            ...
            if (!$entry->get_permission('canmove')) {
                error_log('[Use-your-Drive message]: ' . sprintf('Failed to move %s as the sharing permissions on it prevent this.', $target_entry->get_path()));
                $entries_to_move[$cached_entry->get_id()] = false;
                continue;
            }
    ...
    public function update_entry($entry_id, \UYDGoogle_Service_Drive_DriveFile $entry, $_params = array()) {
        ...
        try {
            ...
            $api_entry = $this->get_app()->get_drive()->files->get($entry_id, array("userIp" => $this->_user_ip, 'fields' => $this->apifilefields, 'supportsAllDrives' => true));
            $entry = new Entry($api_entry);
            $cachedentry = $this->get_cache()->add_to_cache($entry);
        } catch (\Exception $ex) {
            error_log('[Use-your-Drive message]: ' . sprintf('Google API Error on line %s: %s', __LINE__, $ex->getMessage()));
    ...
    function add_folder($new_folder_name = null) {
        ...
        try {
            $api_entry = $this->get_app()->get_drive()->files->create($newfolder, array("fields" => $this->apifilefields, "userIp" => $this->_user_ip, 'supportsAllDrives' => true));
    ...
    public function copy_folder_recursive(CacheNode $templatefolder, CacheNode $newfolder) {
        ...
        foreach ($template_folder_children as $cached_child) {
            ...
            if ($child->is_dir()) {
                ...
                $batch->add($this->get_app()->get_drive()->files->create($newchildfolder, array("fields" => $this->apifilefields, "userIp" => $this->_user_ip, 'supportsAllDrives' => true)), $child->get_id());
            } else {
                ...
                $batch->add($this->get_app()->get_drive()->files->copy($child->get_id(), $newfile, array("fields" => $this->apifilefields, "userIp" => $this->_user_ip, 'supportsAllDrives' => true)), $child->get_id());
            }
        }
...
//    }
    ...
    function edit_entry() {
        ...
        if ($entry->is_dir() || $entry->get_can_edit_by_cloud() === false) {
            die('-1');
        }
        ...
        if (empty($edit_link)) {
            error_log('[Use-your-Drive message]: ' . sprintf('Cannot create a editable link %s', __LINE__));
            die();
        }
    ...
    public function stream_entry() {
        ...
        $extension = $entry->get_extension();
        $allowedextensions = array('mp4', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'oga', 'wav', 'webm', 'vtt');
    ...
    public function get_embed_url(CacheNode $cachedentry) {
        ...
        if (in_array($cachedentry->get_entry()->get_extension(), array('jpg', 'jpeg', 'gif', 'png'))) {
            $preview = USEYOURDRIVE_ADMIN_URL . "?action=useyourdrive-embed-image&account_id={$this->get_processor()->get_current_account()->get_id()}&id=" . $cachedentry->get_id();
        }

        $preview = apply_filters('useyourdrive_set_embed_url', $preview, $cachedentry);
    ...
    public function get_edit_url(CacheNode $cachedentry) {
        ...
        /* Check the permissions and set it if possible */
        if (!$this->has_permission($cachedentry, array('writer'))) {
        ...
        $edit_url = apply_filters('useyourdrive_set_edit_url', $edit_url, $cachedentry);

        return $edit_url;
    }

    public function has_permission(CacheNode $cachedentry, $permission_role = array('reader', 'writer'), $force_update = false) {
        ...
        if (in_array('reader', $permission_role)) {
            ...
            $this->get_library()->getIo()->setOptions(array(CURLOPT_FOLLOWLOCATION => 0));
            $httpRequest = $this->get_library()->getIo()->makeRequest($request);
            curl_close($this->get_library()->getIo()->getHandler());

            if ($httpRequest->getResponseHttpCode() == 200) {
                $users['anyoneWithLink'] = array('type' => 'anyone', 'role' => 'reader', 'domain' => $permission_domain);
    ...
    public function set_permission(CacheNode $cachedentry, $permission_role = 'reader') {
        ...
        if ($this->get_processor()->get_setting('manage_permissions') === 'Yes' && $cachedentry->get_entry()->get_permission('canshare')) {
            ...
            try {
                ...
                $users[$updated_permission->getId()] = array('type' => $updated_permission->getType(), 'role' => $updated_permission->getRole(), 'domain' => $updated_permission->getDomain());
        ...
        if (count($dirlisting['folders']) > 0 || count($dirlisting['files']) > 0) {
            ...
            if (count($dirlisting['files']) > 0) {
                ...
                foreach ($dirlisting['files'] as $key => $file) {
                    ...
                    $this->get_library()->getIo()->setOptions(
                            array(
                    ...
                    try {
                        ...
                        $this->get_library()->getAuth()->authenticatedRequest($request);
                    ...
                    try {
                        $zip->addLargeFile($download_stream, $file['path']);
                    } catch (\Exception $ex) {
