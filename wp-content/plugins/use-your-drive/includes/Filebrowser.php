        ...
        if ($root_folder_id === $current_id) {
            ...
            foreach ($folder_path as $parent_id) {
                ...
                if ($parent_id === $root_folder_id) {
                    ...
                    $file_path .= "<li><a href='javascript:void(0)' class='folder' data-id='" . $parent_id . "'>" . $parent_folder['folder']->get_name() . "</a></li>";
                }
            }
...
<div class="entry_name"><a class="entry_link"><div class="entry-name-view"><span><strong>' . __('No files or folders found', 'useyourdrive') . '</strong></span></div></a></div>
    ...
    public function renderFileForList(EntryAbstract $item) {
        ...
        $thumbnail_small = (strpos($item->get_thumbnail_small(), 'useyourdrive-thumbnail') === false) ? $item->get_thumbnail_small() : $item->get_thumbnail_small() . '&account_id='.$this->get_processor()->get_current_account()->get_id().'&listtoken=' . $this->get_processor()->get_listtoken();
        ...
        $caption = ($this->get_processor()->get_user()->can_download()) ? '<a href="' . $download_url . '" title="' . __('Download', 'useyourdrive') . '" download="' . htmlspecialchars($item->get_name(), ENT_QUOTES) . '"><i class="fas fa-arrow-circle-down" aria-hidden="true"></i></a>&nbsp' : '';
        ...
        if (!empty($thumbnail_small)) {
            ...
            $return .= "<img class='preloading hidden' src='" . USEYOURDRIVE_ROOTPATH . "/css/images/transparant.png' data-src='" . $thumbnail_small . "' data-src-backup='" . $item->get_icon() . "' width='150'/>";
            $return .= "</div>";
        }

        $return .= "<div class='entry_name'>" . $link['filename'];

        if (($this->get_processor()->get_shortcode_option('mcepopup') === 'shortcode') && (in_array($item->get_extension(), array('mp4', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'oga', 'wav', 'webm')))) {
    ...
    public function renderFileForGrid(EntryAbstract $item) {
        ...
        $return .= "<div class='entry_thumbnail'><div class='entry_thumbnail-view-bottom'><div class='entry_thumbnail-view-center'>\n";
        $thumbnail_small = (strpos($item->get_thumbnail_small(), 'useyourdrive-thumbnail') === false) ? $item->get_thumbnail_with_size('w500-h375-p-k') : $item->get_thumbnail_small() . '&account_id='.$this->get_processor()->get_current_account()->get_id().'&listtoken=' . $this->get_processor()->get_listtoken();
        $return .= "<div class='preloading'></div>";
        $return .= "<img class='preloading' src='" . USEYOURDRIVE_ROOTPATH . "/css/images/transparant.png' data-src='" . $thumbnail_small . "' data-src-retina='" . $thumbnail_small . "' data-src-backup='" . str_replace('/64/', '/256/', $item->get_icon()) . "'/>";
        $return .= "</div></div></div>\n";
