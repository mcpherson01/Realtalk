...
//      }
    ...
    static public function get_user_name() {
        ...
        if (!is_user_logged_in()) {
            return __('Anonymous user', 'useyourdrive');
        }
    ...
    static public function get_all_users_and_roles() {
        /* Build list */
        $list = array();
        ...
        /* Add custom roles */
        $wp_roles_names['guest'] = __('Anonymous user', 'useyourdrive');
        $wp_roles_names['all'] = __('Everyone', 'useyourdrive');
        $wp_roles_names['none'] = __('None', 'useyourdrive');

        foreach ($wp_roles_names as $wp_role_id => $wp_role_name) {
            $list[] = array('value' => (string) $wp_role_id, 'id' => (string) $wp_role_id, 'text' => $wp_role_name, 'type' => 'role', 'searchBy' => $wp_role_name, 'img' => get_avatar_url('role', array('size' => '32')));
        }

        /* Get Users */
        $users = get_users(array('fields' => array('user_login', 'display_name', 'id')));

        foreach ($users as $wp_user) {
            $list[] = array('value' => (string) $wp_user->id, 'id' => (string) $wp_user->id, 'text' => $wp_user->display_name, 'type' => 'user', 'searchBy' => $wp_user->user_login, 'img' => get_avatar_url($wp_user->id, array('size' => '32')));
        }

        /* Sort the list by name ASC. First Roles, than Users */
        $sort = array();
        foreach ($list as $k => $v) {
            $sort['is_role'][$k] = ($v['type'] === 'role');
            $sort['sort'][$k] = $v['text'];
        }
     ...
     */
    static function extract_email_from_string($string) {
    ...
    static public function compress_html($minify) {
     ...
     */
    static function convert_ms_to_time($ms, $seconds = false) {
    ...
    static function get_mimetype($extension = '') {
