<div class="list-container" style="width:<?php echo $this->options['maxwidth']; ?>;max-width:<?php echo $this->options['maxwidth']; ?>;">
  ...
  if ($this->options['show_breadcrumb'] === '1' || $this->options['search'] === '1' || $this->options['show_refreshbutton'] === '1' ||
      ...
      ?>
      <div class="nav-header"><?php if ($this->options['show_breadcrumb'] === '1') { ?>
            ...
            <div class="gear-menu" data-token="<?php echo $this->listtoken; ?>">
              <ul data-id="<?php echo $this->listtoken; ?>">
                ...
                if ($this->get_user()->can_add_folders()) {
                    ?>
                    <li><a class="nav-new-folder newfolder" title="<?php _e('Add folder', 'useyourdrive'); ?>"><i class="fas fa-folder-plus fa-lg"></i><?php _e('Add folder', 'useyourdrive'); ?></a></li>
                    <?php
                }
                ...
                if ($this->get_user()->can_download_zip()) {
                    ?>
                    <li><a class="all-files-to-zip" download><i class='fas fa-archive fa-lg'></i><?php _e('Download all', 'useyourdrive'); ?> (.zip)</a></li>
                    <li><a class="selected-files-to-zip" download><i class='fas fa-archive fa-lg'></i><?php _e('Download selected', 'useyourdrive'); ?> (.zip)</a></li>
                    <?php
                }

                if ($this->get_user()->can_move_folders() || $this->get_user()->can_move_files()) {
                    ?>
                    <li><a class='selected-files-move' title='" <?php _e('Move to', 'useyourdrive'); ?>"'><i class='fas fa-folder-open fa-lg'></i><?php _e('Move to', 'useyourdrive'); ?></a></li>
                    <?php
                }
        ...
        if ($this->options['single_account'] === '0') {
