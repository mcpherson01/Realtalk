...
class Ip2Country6 {
    ...
    public function __construct ($datafile = null) {
      if (!$datafile) $datafile = 'ip2country6.dat';
...
function ip_to_country_webnet77 ($ip) {
  $result = null;
  if (strpos ($ip, ':') === false) {
    if (file_exists (__DIR__.'/ip2country.dat')) {
      $ipc = new Ip2Country (__DIR__.'/ip2country.dat');
//      $ipc->preload();
      try {
      ...
      if (file_exists (__DIR__.'/ip2country6.dat')) {
        $ipc = new Ip2Country6 (__DIR__.'/ip2country6.dat');
//        $ipc->preload();
        try {
          $result = $ipc->lookup ($ip);
        } catch (Exception $e) {
...
function ip_to_country ($ip, $details = false) {
  ...
  if (isset ($_SERVER ['HTTP_CF_IPCOUNTRY']) && !$details) {
    return strtoupper ($_SERVER ['HTTP_CF_IPCOUNTRY']);
  }

  if (defined ('AI_MAXMIND_DB')) {
    return ip_to_country_maxmind ($ip, AI_MAXMIND_DB, $details);
  }

  return ip_to_country_webnet77 ($ip);
}
