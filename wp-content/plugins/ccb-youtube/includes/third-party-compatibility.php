...
class CBC_Third_Party_Compat{
	...
	/**
	 * Store object reference for active WP theme
	 * 
	 * @var WP_Theme
	 */
	...
	/**
	 * Store details of the compatible theme set by user
	 * 
	 * @var array
	 */
	...
	public function __construct(){
		...
		if( $this->compatible_theme ){
			...
			if( array_key_exists( 'extra_meta', $this->compatible_theme ) ){
				add_action( 'cbc_post_insert', array( 
						$this, 
						'add_extra_meta' 
				), 30, 3 );
			}
	...
	private function check_compatibility(){
		...
		$theme_name = strtolower( $this->active_theme->get( 'Name' ) );
		/**
		 * Filter for third party code
		 * 
		 * @param array $themes
		 *
		 * @deprecated 1.3 Deprecated in favor of a more flexible way of making compatibility.
		 * @see https://wpythub.com/documentation/tutorials/third-party-compatibility/
		 */
		$themes = apply_filters( 'cbc_youtube_theme_support', $this->wp_themes() );
		
		/**
		 * Filter that allows current theme to be compatible with the plugin.
		 * Should return true if theme has compatibility code in it.
		 */
		$current_theme_support = apply_filters( 'cbc_compatibility', false );
		if( true === $current_theme_support ){
			$theme = array( 
					/**
					 * Filter that is used to tell the plugin which post type the theme uses.
					 * 
					 * @param string
					 */
					'post_type' => apply_filters( 'cbc_compatibility_post_type', false ), 
					/**
					 * Filter that sets the post type category taxonomy
					 * 
					 * @param string
					 */
					'taxonomy' => apply_filters( 'cbc_compatibility_taxonomy', false ), 
					/**
					 * Filter that sets post type tag taxonomy
					 * 
					 * @param string
					 */
					'tag_taxonomy' => apply_filters( 'cbc_compatibility_tag_taxonomy', false ), 
					'post_meta' => array( 
							/**
							 * Filter used to fill a custom field with the video URL
							 * 
							 * @param string
							 */
							'url' => apply_filters( 'cbc_compatibility_url_meta', false ), 
							/**
							 * Filter used to fill a custom field with video embed code
							 * 
							 * @param string
							 */
							'embed' => apply_filters( 'cbc_compatibility_embed_meta', false ), 
							/**
							 * Filter used to fill a custom field with video thumbnail URL
							 * 
							 * @param string
							 */
							'thumbnail' => apply_filters( 'cbc_compatibility_image_meta', false ), 
							/**
							 * Filter used to fill a custom field with video number of likes
							 * 
							 * @param string
							 */
							'likes' => apply_filters( 'cbc_compatibility_likes_meta', false ), 
							/**
							 * Filter used to fill a custom field with video number of views
							 * 
							 * @param string
							 */
							'views' => apply_filters( 'cbc_compatibility_views_meta', false ), 
							/**
							 * Filter used to fill a custom field with video number of dislikes
							 * 
							 * @param string
							 */
							'dislikes' => apply_filters( 'cbc_compatibility_dislikes_meta', false ), 
							/**
							 * Filter used to fill a custom field with video duration in seconds
							 * 
							 * @param string
							 */
							'duration' => apply_filters( 'cbc_compatibility_duration_meta', false ), 
							/**
							 * Filter used to fill a custom field with formatted video duration (ie.
							 * HH:mm:ss)
							 * 
							 * @param string
							 */
							'human_duration' => apply_filters( 'cbc_compatibility_human_duration_meta', false ) 
					), 
					/**
					 * Filter used to set the post format for videos imported for theme
					 */
					'post_format' => apply_filters( 'cbc_compatibility_post_format', 'video' ), 
					/**
					 * Filter used to alter the theme name
					 * 
					 * @param string
					 */
					'theme_name' => apply_filters( 'cbc_compatibility_theme_name', $this->active_theme->get( 'Name' ) ), 
					/**
					 * Filter used to alter the theme URL
					 * 
					 * @param string
					 */
					'url' => apply_filters( 'cbc_compatibility_theme_url', $this->active_theme->get( 'ThemeURI' ) ) 
			);
			
			$this->compatible_theme = $theme;
			return true;
		}
		...
		$themes = array( 
				...
				'wave' => array( 
						...
						'extra_meta' => array( 
								'video_width_value' => array( 
										...
										'value' => 'width' 
								),  // value needed is stored under key width in plugin settings
				...
				'detube' => array( 
						...
						'post_meta' => array( 
								...
								'thumbnail' => 'dp_video_poster' 
						),  // custom fiels on post to save video thumbnail for detube compatibility
				...
				),  // for seconds duration, use only duration
				...
				'betube' => array( 
						...
						'post_format' => 'video', 
						'theme_name' => 'Betube', 
						'url' => 'https://themeforest.net/item/betube-video-wordpress-theme/16437378/?ref=cboiangiu', 
						'extra_meta' => array( 
 ...
 */
function cbc_filter_yoast_sitemap_type( $type ){
	if( ! class_exists( 'wpseo_Video_Sitemap' ) ){
		return $type;
	}
	
	$obj = cbc_get_class_instance();
	$post_type = $obj->get_post_type();
...
add_filter( 'wpseo_build_sitemap_post_type', 'cbc_filter_yoast_sitemap_type', 10, 1 );
 ...
 */
/*
function cbc_ultimate_seo_fields( $post_id, $video, $theme_import ){
...
add_action('cbc_post_insert', 'cbc_ultimate_seo_fields', 10, 3);
*/