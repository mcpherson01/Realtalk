...
class CBC_Review_Callout{
	 ...
	 * @param number $delay
	 */
	public function __construct( $option_name, CBC_Message $message, CBC_User $user, $delay = 7 ){
...
class CBC_Message{
	...
	/**
	 * Saves the CBC_User instance needed to display message actions
	 * 
	 * @param CBC_User $user
	 */
	public function set_user( CBC_User $user ){
		$this->user = $user;
	}
...
<?php
	...
	private function note_footer( $echo = true ){
		...
		$links = array( 
				sprintf( $template, $this->review_url, __( "Sure, I'd love to!", 'cbc_video' ) ), 
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'yes' ) ) ), __( 'No, thanks.', 'cbc_video' ) ), 
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'yes' ) ) ), __( "I've already given a review.", 'cbc_video' ) ), 
				sprintf( $template, esc_url( add_query_arg( $this->user->get_query_arg( 'later' ) ) ), __( 'Ask me later.', 'cbc_video' ) ) 
		);
