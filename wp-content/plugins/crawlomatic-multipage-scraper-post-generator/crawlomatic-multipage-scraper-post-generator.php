...
if(is_admin())
{
    ...
    else
    {
        add_action("after_plugin_row_{$plugin}", function( $plugin_file, $plugin_data, $status ) {
            $plugin_url = '//codecanyon.net/item/crawlomatic-multisite-scraper-post-generator-plugin-for-wordpress/20476010';
...
function crawlomatic_iframe_callback() {
        ...
        if(isset($_GET['crawl_children']) && $_GET['crawl_children'] != '' && $_GET['crawl_children'] != 'false' && isset($_GET['crawl_children_expression']) && $_GET['crawl_children_expression'] != '' && $_GET['crawl_children_expression'] != 'false')
        {
            ...
            if(isset($anchors[0]))
            {
                ...
                $new_url = crawlomatic_generate_valid_url($new_url, $url, '1', $crawlomatic_Main_Settings);
...
function crawlomatic_curl_exec_utf8($ch) {
    ...
    if (!is_string($data))
    {
        ...
        crawlomatic_log_to_file('Failed to exec curl in crawlomatic_curl_exec_utf8! ' . $eff_url . ' - err: ' . curl_error($ch) . ' - ' . curl_errno($ch));
        return $data;
    } 
    unset($charset);
...
function crawlomatic_get_web_page($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass = '')
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if (isset($crawlomatic_Main_Settings['request_timeout']) && $crawlomatic_Main_Settings['request_timeout'] != '') {
        $timeout = intval($crawlomatic_Main_Settings['request_timeout']);
                ...
                if ($get_css == '1') {
                    ...
                    if ($htmlcontent !== FALSE) {
                        ...
                        foreach ($matches as $match) {
                            ...
                            if (!crawlomatic_url_exists($match, $use_proxy, $crawlomatic_Main_Settings)) {
                                $tmp_match = 'http:' . $match;
                                if (!crawlomatic_url_exists($tmp_match, $use_proxy, $crawlomatic_Main_Settings)) {
                                    $parts = explode('/', $url);
                                    $dir   = '';
...
add_filter('post_link','crawlomatic_permalink_changer');
add_filter('the_permalink','crawlomatic_permalink_changer');
function crawlomatic_permalink_changer($link){
	global $post;	 
	if (!empty($post->ID)) {
        $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
        if (isset($crawlomatic_Main_Settings['crawlomatic_enabled']) && $crawlomatic_Main_Settings['crawlomatic_enabled'] == 'on') {
            if (isset($crawlomatic_Main_Settings['link_source']) && $crawlomatic_Main_Settings['link_source'] == 'on') {
                $url = get_post_meta($post->ID, 'crawlomatic_change_title_link', true);
                if ( trim($url) == '1')
                {
                    $new_url = get_post_meta($post->ID, 'crawlomatic_post_url', true);
                    if(trim($new_url) != '') {
                        return $new_url;
                    }
                }
...
function crawlomatic_get_free_image($crawlomatic_Main_Settings, $query_words, &$img_attr, $res_cnt = 3)
{
    ...
    while(($rez === false || $rez === '') && count($rand_arr) > 0)
    {
        ...
        {
            unset($rand_arr[$rand]);
            $rez = crawlomatic_get_pixabay_image($crawlomatic_Main_Settings['pixabay_api'], $query_words, $crawlomatic_Main_Settings['img_language'], $crawlomatic_Main_Settings['imgtype'], $crawlomatic_Main_Settings['img_orientation'], $crawlomatic_Main_Settings['img_order'], $crawlomatic_Main_Settings['img_cat'], $crawlomatic_Main_Settings['img_mwidth'], $crawlomatic_Main_Settings['img_width'], $crawlomatic_Main_Settings['img_ss'], $crawlomatic_Main_Settings['img_editor'], $original_url, $res_cnt);
            if($rez !== false && $rez !== '')
            {
...
function crawlomatic_best_spin_text($title, $content)
{
    ...
    if ($output['success'] == 'true') {
        ...
        if ($ch === FALSE) {
            if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                crawlomatic_log_to_file('"The Best Spinner" failed to init curl after auth.');
            }
            return FALSE;
        }
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        $output = crawlomatic_curl_exec_utf8($ch);
...
function crawlomatic_replaceExecludes($article, &$htmlfounds, $opt = false)
{
...
function crawlomatic_translate($title, $content, $from, $to, $use_proxy = '1')
{
    ...
    try {
        ...
        curl_close($ch);
        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
            crawlomatic_log_to_file('Exception thrown in GoogleTranslator ' . $e);
        }
        return false;
    }
...
function crawlomatic_url_exists($url, $use_proxy, $crawlomatic_Main_Settings)
{
...
function crawlomatic_crawl_page($url, $max, $skip_og, $skip_post_content, $no_external, $required_words, $banned_words, $type, $getname, $title_type, $title_getname, $image_type, $image_getname, $date_type, $date_getname, $cat_type, $cat_getname, $depth = 2, $custom_cookies, $only_class, $only_id, $no_source, $seed_type, $seed_expre, $crawled_type, $crawled_expre, $paged_crawl_str, $paged_crawl_type, $max_paged_depth, $custom_user_agent, $posted_items, $update_ex, $cat_sep, $root_page, $seed_pag_type, $seed_pag_expre, $price_type, $price_expre, $clear_static = false, $use_proxy = '1', $use_phantom = '0', $no_dupl_crawl = false, $custom_crawling_expre, $user_pass, $crawl_exclude = array(), $crawl_title_exclude = array(), $price_sep, $encoding, $strip_comma, $reverse_crawl, $lazy_tag, $tag_type, $tag_expre, $tag_sep, $phantom_wait)
{
    ...
    if($depth - 1 > 0 && $seed_type != 'disabled')
    {
        ...
        foreach ($anchors as $element) {
            ...
            $href = crawlomatic_generate_valid_url($href, $url, $use_proxy, $crawlomatic_Main_Settings);
            if($no_external == '1')
            {
    ...
    if($root_page === true && count($posts) < $max && $seed_pag_type !== 'disabled' && $seed_type !== 'disabled')
    {
        ...
        if($seed_pag_expre != '' && $seed_pag_type != '' && $seed_pag_type != '1')
        {
            if ($seed_pag_type == 'xpath' || $seed_pag_type == 'visual') {
                ...
                foreach($elements as $el) {
                    if(isset($el->tagName) && $el->tagName === 'a')
                    {
                        ...
                        $href_val = crawlomatic_generate_valid_url($href_val, $url, $use_proxy, $crawlomatic_Main_Settings);
                        if($href_val != '')
                        {
            ...
            elseif ($seed_pag_type == 'regex') {
                ...
                if(isset($matches[0]))
                {
                    foreach ($matches[0] as $match) {
                        $el = $dom->createElement('a', 'link');
                        $href_val = crawlomatic_generate_valid_url(trim($match), $url, $use_proxy, $crawlomatic_Main_Settings);
                        if($href_val != '')
                        {
        ...
        foreach($xanchors as $ancas)
        {
            ...
            $ancas = crawlomatic_generate_valid_url($ancas, $url, $use_proxy, $crawlomatic_Main_Settings);
            $GLOBALS['seed'] = true;
            $GLOBALS['crawl_done'] = false;
            if($no_external == '1')
            {
...
function crawlomatic_generate_valid_url($href, $root, $use_proxy, $crawlomatic_Main_Settings)
{
    $newx_url = $href;
    ...
    if (0 !== strpos($newx_url, 'http')) 
    {
        ...
        if (function_exists('http_build_url')) {
            ...
            if(!crawlomatic_url_exists($newx_url, $use_proxy, $crawlomatic_Main_Settings))
            {
...
function crawlomatic_get_content($type, $getname, $htmlcontent, $single = false)
{
    ...
    if ($type == 'regex') {
        ...
        {
            if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                crawlomatic_log_to_file('crawlomatic_str_get_html failed for page, find does not exist: ' . print_r($html_dom_original_html, true) . '!');
            }
            return '';
        }
    }
