...
abstract class ShopParser {
    ...
    protected $user_agent = 'Mozilla/5.0 (Windows NT 6.3; rv:36.0) Gecko/20100101 Firefox/36.0';
    ...
    public function parseProduct()
    {
        ...
        $this->item['description'] = \wp_encode_emoji(\sanitize_text_field(html_entity_decode($this->parseDescription())));
        ...
        foreach ($this->item['extra']['comments'] as $i => $comment)
        {
            if (isset($comment['name']))
                $this->item['extra']['comments'][$i]['name'] = \wp_encode_emoji(\sanitize_text_field($comment['name']));
            $this->item['extra']['comments'][$i]['comment'] = \wp_encode_emoji(\sanitize_text_field($comment['comment']));
        }
        return $this->item;
    }
    ...
    public function decodeCharset($str, $fix_encoding = true)
    {
        ...
        if (strtolower($this->charset) != 'utf-8')
        {
            ...
            $result = iconv($this->charset, 'utf-8', $str);
        } else
